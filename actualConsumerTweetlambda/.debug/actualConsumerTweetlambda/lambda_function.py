...
#SETUP Twitter API constants
CONSUMER_KEY = "fvBGhF7LjEv2Um8b7JzsIxO6l"
CONSUMER_SECRET = "yd3DCz1G6pgPFHhC5SqRoOqPuiWICIKHZmuK8sxq7Gg5C1dMZz"
ACCESS_TOKEN = "1370183630-FnM7LhgZy8VJFSIpvzheSkyR2ShFTlqsieqBune"
ACCESS_TOKEN_SECRET = "qo5viQoWAWReiS4GEvSLeCKvaHxji15Q9AIIj4iEqh26B"
...
def pull_new_tweets(twitterhandles):
    ...
    try:
        ...
        LOG.error("Error authenticating twitter api", exc_info=True)
        raise ex
    LOG.info("Authentication Successful. API Created")
    
    #create data dictionary for use in dataframe creation
    data_dict ={'handles':[], 'tweets':[]}
    
    # Read top 20 timelines for each twitter handle
    for handle in twitterhandles:
        
        HandleTimeline = api.user_timeline(screen_name=handle, tweet_mode="extended")
    
        current_tweets =""
        
        for tweet in HandleTimeline:
            current_tweets = current_tweets + tweet.full_text
            
            LOG.info(f"tweets for {handle} : {current_tweets}")
            
        #append values into dictionary
        data_dict['handles'].append(handle)
        data_dict['tweets'].append(current_tweets)
        
    df = pd.DataFrame(data_dict)
    #LOG.info(f"dataframe info:  handle = {df.handles} ; tweets = {current_tweets}")
...
def apply_sentiment(df, column="tweets"):
...
def lambda_handler(event, context):
    ...
    twitterhandles = [] #Captured from Queue

    # Process Queue
    for record in event['Records']:
        body = json.loads(record['body'])
        twitter_handles = body['guid']

        #Capture for processing
        twitterhandles.append(twitter_handles)

        extra_logging = {"body": body, "twitter_handle":twitter_handles}
    ...
    LOG.info(f"Creating dataframe with values: {twitterhandles}")
    df = pull_new_tweets(twitterhandles)
    LOG.info(f"Dataframe successfully created for handles: {twitterhandles}")
    LOG.info(f"dataframe shape: {df.shape}")
    # Perform Sentiment Analysis
    df = apply_sentiment(df)
    LOG.info(f"Sentiment from twitter handles: {df.to_dict()}")

    # Write result to S3
    write_s3(df=df, bucket="twitterhandlesentiments", name=twitterhandles)
